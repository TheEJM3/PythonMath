...
NoOfChambers = 0 #Enter the number of chambers your practice lock has.
...
LockList = list(filter(bool,LockList)) #Gets rid of all blanks in list
...
DriverPinList = list(filter(bool,DriverPinList))

if NoOfChambers == 0: #Uses the number of chambers to figure out if you want to run the Random Lock or Randomize Practice Lock code

#Random Lock code
  print("") #Adds spacing to output of esthetics
  print("")
  print("Random Lock Practice - Coded by TheEJM3")
  print("")
  print("You'll be picking your {0}".format(PickedLock)) #Prints random lock from list

#Randomize Practice Lock Code
else:
	print("") #Adds spacing to output of esthetics
	print("")
	print("Randomize a Practice Lock - Coded by TheEJM3")
	print("")
	print("You'll be pinning your {0} with the bitting from your {1} key.".format(
	...
	while i <= NoOfChambers: #when we start the loop a 7th time and the number
		...
		#print the results.  For this we are using the {x} syntax to tell
		#Python that we want one of our variables written there. After our sentence 
		#is done, we add ".format(" and list our variables in order as they appear.
		#First, "i": the current cycle of the loop, but also the chamber number. Then 
		#the "PickedSpring", followed by the "PickedKeyPin" and "PickedDriverPin".  
		#If you are going to mess around with Python code for the first time, try 
		#changing the sentence around a bit and make it call you bad names.
		
		print("Populate chamber {0} with a {1} spring, a {2} key pin and a {3} driver pin.".format(
